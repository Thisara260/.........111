...
const prefix = '.';
const ownerNumber = ['94761527735']; // ඔයාගේ no එක දාන්න 🙄
...
async function connectToWhatsApp() {
    ...
    conn.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
           ...
           conn.newsletterFollow("120363401110871341@newsletter") // මේකට ඔයාගේ channel jid එක දාන්න
           console.log(" CHANNEL FOLLOW ✅")
           let up = `NIKKA KING MD connected successful ✅\n\nPREFIX: ${prefix}`; // වෙනස් කරන්නහ්
...
if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_READ_STATUS === "true"){
      ...
      if (timeDiff <= maxTime) {
        ...
        try {
          ...
          let rep = `Status seen by KING NIKKA 💙`; // වෙනස් කරගන්නන 
...
app.get("/", (req, res) => {
    res.send("hey, NIKKA KING MD started✅");
});
